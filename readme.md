...
- **🔌 SSH 远程连接**：基于 SSH2 协议，支持密码和密钥两种认证方式
- **🗂️ 自动备份**：支持部署前自动备份，保留历史版本
- **👥 集群部署**：支持同时部署到多台服务器
- **🔔 钉钉通知**：部署开始和完成自动发送钉钉消息提醒
- **🌐 自动预览**：部署完成后自动打开预览地址
- **🔑 密钥认证**：支持 SSH 私钥文件认证，可选配置密钥密码，比密码更安全
...
- 项目名称设置
- 多环境选择（dev/test/prod）
- 服务器连接配置（host/port/username/privateKeyPath/password）
- 打包命令配置
- 本地打包目录和远程部署路径配置
- 备份策略配置
- 钉钉通知配置
...
module.exports = {
  ...
  dev: {
    ...
    port: 22,
    username: "root",
    privateKeyPath: "",        // SSH 密钥文件路径，填写后优先使用密钥认证
    privateKeyPassphrase: "",  // 密钥文件密码（密钥无密码留空）
    password: "your-password", // 未配置密钥时使用密码认证
    distPath: "dist",
    webDir: "/var/www/html",
    isBak: true,
...
| **privateKeyPath**     | `string`  | ❌   | -       | SSH 密钥文件路径，填写后优先使用密钥认证 |
| **privateKeyPassphrase** | `string` | ❌  | -       | SSH 密钥文件密码（密钥无密码留空）    |
| **password**           | `string`  | 条件 | -       | 服务器密码（未配置密钥时必填）        |
...
相比密码认证，使用 SSH 密钥更安全，推荐在生产环境中使用：
...
1. **推荐使用密钥认证**：配置 `privateKeyPath` 使用 SSH 密钥替代密码，更安全且无需在配置文件中保存密码

2. **密码安全**：若使用密码认证，配置文件中包含服务器密码，请确保：
   - 不要将 `hdeploy.config.js` 提交到版本控制
   - 在 `.gitignore` 中添加配置文件

3. **服务器权限**：确保部署用户对目标目录有读写权限
...
# 检查是否能手动连接 SSH（密钥）
ssh -i /path/to/id_rsa root@your-server-ip -p 22

# 确认配置文件中的 host、port、username 正确
# 使用密钥认证时，确认 privateKeyPath 路径正确且公钥已添加到服务器
```
